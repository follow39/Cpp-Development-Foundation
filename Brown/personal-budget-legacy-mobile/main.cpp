...
array<double, VERTEX_COUNT> tree_values;
array<double, VERTEX_COUNT> tree_add;
array<double, VERTEX_COUNT> tree_factor;
array<double, VERTEX_COUNT> arr_sub;
...
double ComputeSub(size_t from, size_t to) {
    double result = 0.0;
...
void Multiply(size_t v, size_t l, size_t r, size_t ql, size_t qr, double tax) {
    ...
    Push(v, l, r);
    if (ql <= l && r <= qr) {
        tree_factor[v] *= 1 - tax / 100;
        tree_add[v] *= 1 - tax / 100;
        tree_values[v] *= 1 - tax / 100;
        return;
    }
...
void Spend(size_t ql, size_t qr, double value) {
...
int main() {
    ...
    for (int i = 0; i < q; ++i) {
        ...
        if (query_type == "ComputeIncome") {
            double sum = ComputeSum(1, 0, DAY_COUNT_P2, idx_from, idx_to);
            double sub = ComputeSub(idx_from, idx_to);
            double res = sum - sub;
            cout << res << endl;
            ...
            double value;
            cin >> value;
            Spend(idx_from, idx_to, value / (idx_to - idx_from));
        }
    }
